...
def test_playwright_proxy_can_force_scheme():
    mgr = _make_manager(["http://user:pass@proxy1:8080"])
    result = mgr.playwright_proxy(scheme="socks5")
    assert result == {"server": "socks5://user:pass@proxy1:8080"}
...
def test_no_proxy_configured():
    with patch("src.proxy.settings") as mock_settings:
        mock_settings.proxy_file = ""
        mock_settings.proxy_url = ""
        mock_settings.proxy_storage_dir = ""
        mgr = ProxyManager()
...
def test_scope_uses_its_own_default_file(tmp_path):
    storage_dir = tmp_path / "proxies"
    storage_dir.mkdir()
    (storage_dir / "avito.txt").write_text("http://avito-proxy:8080\n", encoding="utf-8")
    (storage_dir / "yandex.txt").write_text("http://yandex-proxy:8080\n", encoding="utf-8")

    with patch("src.proxy.settings") as mock_settings:
        mock_settings.proxy_storage_dir = str(storage_dir)
        mock_settings.avito_proxy_file = ""
        mock_settings.avito_proxy_url = ""
        mock_settings.yandex_proxy_file = ""
        mock_settings.yandex_proxy_url = ""
        avito_mgr = ProxyManager(name="avito")
        yandex_mgr = ProxyManager(name="yandex")

    assert avito_mgr.get_next() == "http://avito-proxy:8080"
    assert yandex_mgr.get_next() == "http://yandex-proxy:8080"
