"""Otzovik review parser with proxy-only HTTP access."""
...
class OtzovikParser(AbstractReviewParser):
    ...
    async def parse(self, url: str, company_name: str) -> list[RawReview]:
        for proxy_url in iter_proxy_urls(enrichment_proxy_manager, purpose="otzovik parser"):
            async with httpx.AsyncClient(
                ...
                try:
                    ...
                    log.warning("otzovik.fetch.error", url=url, error=str(exc), proxy=proxy_url)
                    continue

                if response.status_code == 403:
                    log.info("otzovik.cloudflare_blocked", url=url, proxy=proxy_url)
                    continue
                if response.status_code != 200:
                    log.debug("otzovik.non_200", url=url, status=response.status_code, proxy=proxy_url)
                    continue

                reviews = self._parse_page(response.text, url)
                log.info("otzovik.parse.done", url=url, count=len(reviews), proxy=proxy_url)
                return reviews
    ...
    def _parse_page(self, html: str, page_url: str) -> list[RawReview]:
        ...
        for block in soup.select("div.review-body, div[class*='review-item']")[:20]:
            ...
            if rating_el:
                value = rating_el.get("value") or rating_el.get_text(strip=True)
                try:
